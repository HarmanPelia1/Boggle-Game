        ...
        totalProbes_(0), mIndex_(0), del_count(0), size_(0)
{
    // Initialize any other data members as necessary
        ...
        table_.resize(CAPACITIES[mIndex_], 0);
}
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
	    for (auto& item_ptr : table_) delete item_ptr;
     table_.clear();
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    std::vector<HashItem*> previousTable = table_;
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        else if(table_[loc] -> deleted != true && kequal_(table_[loc]->item.first, key) ){
            return loc;
        }
        loc = prober_.next();
        totalProbes_++;
